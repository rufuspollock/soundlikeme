...
9. **Vendor humanizer's pattern list in place of `tells.md`.** [docs/humanizer-vs-soundlikeme.md](docs/humanizer-vs-soundlikeme.md) ran `polish` against `blader/humanizer` on one document: both strip the same tells identically, and every difference traced to the stored profile. humanizer's list (MIT, ~39k stars, tracks the Wikipedia page) is better maintained than ours and maintaining both in parallel just lets them drift. Keep `protections.md` and the profile machinery — that is the differentiator — and re-run the pairwise ablation after the swap to confirm no voice regression.
...
- **Growing the banlist.** It has converged across all nine surveyed projects and avoid-ai-writing has 100KB of it under MIT. Adding entries is not progress here. Adding a fixture for an entry that already exists is. See item 9 above: the live question is whether to keep our own list at all or vendor humanizer's.
